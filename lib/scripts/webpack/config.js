...
const webpackConfigFile = mode === 'development' ? path.join(cwd,'webpack.config.dev.js') : (mode === 'test' ? path.join(cwd,'webpack.config.test.js') : path.join(cwd,'webpack.config.prod.js'));
const webpackConfigDef = {};
if(fs.existsSync(webpackConfigFile)){
    ...
    if(typeof webpackConfigDef !=='object'){
...
if( !resolve.modules.includes( node_modules_path ) ){
    defaultModules.push( node_modules_path );
}
if( !resolve.modules.includes( project_config.workspace ) ){
    defaultModules.push( project_config.workspace );
}
resolve.modules.push(...defaultModules);
