...
function create(config){
    ...
    const updateScripts = (plugin)=>{
        if( config.plugins.length === 1 ){
            if( plugin ==="es-javascript"){
                extend(true, packageinfo.scripts, {
                    "start": "npm esc -m development",
                    "build": "npm esc -m production",
                    "test": "npm esc -m test"
                });
        ...
        if( plugin==="es-vue" ){
            extend(true, packageinfo.scripts, {
                ...
                "esc:build": "npm esc -f src/App.es -m production",
            });
        }
    }
    ...
    if(config.plugins.length === 1 && config.plugins[0]==='es-thinkphp'){
        esConfig.input = 'src/app/controller'
    }
