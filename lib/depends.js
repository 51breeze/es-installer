...
function create(config){
    ...
    const pluginOptions = {};
    const getPluginImportId = (id)=>{
        return '@easescript/'+id;
    };
        ...
        if( config.plugins.length === 1 ){
            if( plugin ==="es-javascript"){
                extend(true, packageinfo.scripts, {
                    "dev": "esc -f App.es -m development --watch",
                    "test": "esc -f App.es -m test",
                    "build": "esc -f App.es -m production --emit-types"
                });
                ...
                extend(true, packageinfo.scripts, {
                    "dev": `esc -f ${file} -m development --watch`,
                    "test": `esc -f ${file} -m test`,
                    "build": `esc -f ${file} -m production --emit-types`
                });
                if(plugin==="es-thinkphp"){
                    tasks.push(install.thinkphp(config.build_path));
                }
            }
            pluginOptions[ plugin ]= Object.assign(pluginOptions[ plugin ] || {}, {
                outDir:relativePath(config.build_path, config.project_path),
            });
        }else{
            if( plugin==="es-thinkphp" || plugin==="es-php" ){
                const output = resolvePath(PATH.join(config.build_path, 'server'));
                pluginOptions[ plugin ]=Object.assign(pluginOptions[ plugin ] || {},{
                    outDir:relativePath(output , config.project_path)
                });
                if( plugin==="es-thinkphp" ){
                    tasks.push( install.thinkphp(output) );
                }
            }else{
    ...
    config.plugins.forEach( plugin=>{
        if( plugin==="es-vue" ){
                ...
                pluginOptions[ plugin ]={
                    ...
                    resolve:{
                        folders:{
                            ...
                            'api/model/***':'app/model/{...}',
                            'api/config/***':'config/{...}',
                            'api/validate/*.es':'app/validate/{...}',
                            'api/*.es':'app'
                        }
                    },
                    context:{
                        inherits:['es-php'],
                        common:/([\\\/]|^)enums|locales([\\\/])/,
                        include:[/([\\\/]|^)api([\\\/])/],
                        only:true
                    }
                };
            ...
            pluginOptions[ plugin ]=Object.assign(pluginOptions[ plugin ], {
                devServer:{
                    path:getDescriptor(null, '运行路径，默认为outDir'),
    ...
    const esConfig = {
        ...
        buildDir:relativePath(config.build_path, config.project_path),
        plugins:config.plugins.map( plugin => {
            const options = pluginOptions[plugin]||{};
            return {
                'name':getPluginImportId(plugin),
                'options':options
            };
        })
    };
